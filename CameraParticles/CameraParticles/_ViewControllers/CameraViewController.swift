...
@objc open class CameraViewController: TrackingViewController {
    ...
    open var cameraPermission: CameraPermission? {
        didSet {
            changeObservation(from: oldValue, to: cameraPermission, keyPath: #keyPath(CameraPermission.authorization)) { [weak self] _, _, _ in
    ...
    @objc open dynamic var capture: CameraCapture? {
        didSet {
            changeObservation(from: oldValue, to: capture, keyPath: #keyPath(CameraCapture.previewLayer)) { [weak self] _, _, _ in
                if let self = self {
                    self.previewLayer = self.capture?.previewLayer
                }
            }
            changeObservation(from: oldValue, to: capture, keyPath: #keyPath(CameraCapture.light)) { [weak self] _, _, _ in
                if let self = self {
                    self.updateLight()
                }
            }
    ...
    open func updateLight() {
        if capture?.light ?? false {
            flashButton?.buttonImage = UIImage.named("icon_light_off", bundles: Bundle.particles)
        } else {
            flashButton?.buttonImage = UIImage.named("icon_light_on", bundles: Bundle.particles)
        }
    }
}
