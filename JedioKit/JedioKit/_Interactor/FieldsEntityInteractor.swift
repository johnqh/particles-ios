...
open class FieldsEntityInteractor: NSObject, InteractorProtocol {
    @IBOutlet open var fieldLoader: FieldLoader? {
        didSet {
            changeObservation(from: oldValue, to: fieldLoader, keyPath: #keyPath(FieldLoader.definitionFile)) { [weak self] _, _, _ in
                if let self = self {
