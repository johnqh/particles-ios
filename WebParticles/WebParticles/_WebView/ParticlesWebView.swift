...
@objc open class ParticlesWebView: UIView {
    ...
    public static func setup() {
        if !initialized {
            if let view = ViewControllerStack.shared?.root()?.view, let url = URL(string: "https://beansroute.ai") {
    ...
    open var cookieDomain: WebCookieDomain? {
        didSet {
            ...
            changeObservation(from: oldValue, to: cookieDomain, keyPath: #keyPath(WebCookieDomain.location)) { [weak self] _, _, _ in
                self?.locationChanged()
            }
        }
    }

    open var _webView: WKWebView? {
        didSet {
            changeObservation(from: oldValue, to: _webView, keyPath: #keyPath(WKWebView.canGoBack)) { [weak self] _, _, _ in
                if let self = self {
                    self.canGoBack = self._webView?.canGoBack ?? false
                }
            }
            changeObservation(from: oldValue, to: _webView, keyPath: #keyPath(WKWebView.canGoForward)) { [weak self] _, _, _ in
                if let self = self {
                    self.canGoForward = self._webView?.canGoForward ?? false
                }
            }
            changeObservation(from: oldValue, to: _webView, keyPath: #keyPath(WKWebView.title)) { [weak self] _, _, _ in
                if let self = self {
                    self.title = self._webView?.title
                }
            }
            changeObservation(from: oldValue, to: _webView, keyPath: #keyPath(WKWebView.url)) { [weak self] _, _, _ in
                if let self = self {
