...
open class ParticlesWebPresenter: NSObject, WKNavigationDelegate {
    ...
    @IBOutlet @objc open dynamic var webview: ParticlesWebView? {
        didSet {
            webview?.domain = domain
            changeObservation(from: oldValue, to: webview, keyPath: #keyPath(ParticlesWebView.title)) { [weak self] _, _, _ in
                if let self = self {
                    self.title = self.webview?.title
                }
            }
            changeObservation(from: oldValue, to: webview, keyPath: #keyPath(ParticlesWebView.url)) { [weak self] _, _, _ in
                if let self = self {
