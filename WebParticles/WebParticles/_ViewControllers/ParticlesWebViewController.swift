...
open class ParticlesWebViewController: NavigableViewController, WKNavigationDelegate {
    @IBOutlet open var presenter: ParticlesWebPresenter? {
        didSet {
            changeObservation(from: oldValue, to: presenter, keyPath: #keyPath(ParticlesWebPresenter.webview)) { [weak self] _, _, _ in
                self?.setupWebview()
            }
            changeObservation(from: oldValue, to: presenter, keyPath: #keyPath(ParticlesWebPresenter.title)) { [weak self] _, _, _ in
                self?.updateTitle()
            }
            changeObservation(from: oldValue, to: presenter, keyPath: #keyPath(ParticlesWebPresenter.url)) { [weak self] _, _, _ in
                self?.updateUrl()
            }
