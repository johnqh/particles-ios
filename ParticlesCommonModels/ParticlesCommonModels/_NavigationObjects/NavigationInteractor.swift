...
@objc public class NavigationInteractor: NSObject, InteractorProtocol {
    @objc public dynamic var entity: ModelObjectProtocol? {
        didSet {
            changeObservation(from: oldValue, to: entity, keyPath: #keyPath(NavigationModelProtocol.children)) { [weak self] _, _, _ in
                self?.syncChildren()
            }
            changeObservation(from: oldValue, to: entity, keyPath: #keyPath(NavigationModelProtocol.actions)) { [weak self] _, _, _ in
                self?.syncActions()
            }
        }
    }
