...
public class AuthLoginPresenter: ObjectPresenter {
    @objc public dynamic var auth: AuthService? {
        didSet {
            if auth !== oldValue {
                changeObservation(from: oldValue, to: auth, keyPath: #keyPath(AuthService.provider)) { [weak self] _, _, _ in
                    if let self = self {
    ...
    @objc public dynamic var provider: AuthProviderProtocol? {
        didSet {
            changeObservation(from: oldValue, to: provider, keyPath: #keyPath(AuthProviderProtocol.token)) { [weak self] _, _, _ in
