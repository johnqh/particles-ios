...
@objc open class ProgressPresenter: NSObject {
    ...
    @objc open dynamic var progress: ProgressProtocol? {
        didSet {
            changeObservation(from: oldValue, to: progress, keyPath: #keyPath(ProgressProtocol.started)) { [weak self] _, _, _ in
                self?.updateStarted()
            }
            changeObservation(from: oldValue, to: progress, keyPath: #keyPath(ProgressProtocol.progress)) { [weak self] _, _, _ in
                self?.updateProgress()
            }
            changeObservation(from: oldValue, to: progress, keyPath: #keyPath(ProgressProtocol.error)) { [weak self] _, _, _ in
                self?.updateError()
            }
            changeObservation(from: oldValue, to: progress, keyPath: #keyPath(ProgressProtocol.text)) { [weak self] _, _, _ in
                self?.updateText()
            }
        }
    }
