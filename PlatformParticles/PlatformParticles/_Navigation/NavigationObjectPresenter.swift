...
@objc public class NavigationObjectPresenter: ObjectPresenter {
    ...
    @objc override public var model: ModelObjectProtocol? {
        didSet {
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.title)) { [weak self] _, _, _ in
                self?.titleLabel?.text = self?.navigationObject?.title
                self?.actionButton?.buttonTitle = self?.navigationObject?.title
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.subtitle)) { [weak self] _, _, _ in
                self?.subtitleLabel?.text = self?.navigationObject?.subtitle
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.text)) { [weak self] _, _, _ in
                self?.textLabel?.text = self?.navigationObject?.text
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.subtext)) { [weak self] _, _, _ in
                self?.subtextLabel?.text = self?.navigationObject?.subtext
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.color)) { [weak self] _, _, _ in
                self?.colorView?.backgroundColor = ColorPalette.shared.color(system: self?.navigationObject?.color)
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.icon)) { [weak self] _, _, _ in
                self?.iconView?.imageUrl = self?.navigationObject?.icon
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.image)) { [weak self] _, _, _ in
                self?.imageView?.imageUrl = self?.navigationObject?.image
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.children)) { [weak self] _, _, _ in
                self?.syncChildren()
            }
            changeObservation(from: oldValue, to: model, keyPath: #keyPath(NavigationModelProtocol.actions)) { [weak self] _, _, _ in
                self?.syncActions()
            }
        }
    }
