...
public class XibActionPresenter: ObjectPresenter {
    ...
    override public var model: ModelObjectProtocol? {
        didSet {
            changeObservation(from: oldValue, to: action, keyPath: #keyPath(XibAction.title)) { [weak self] _, _, _ in
                self?.updateTitle()
            }
            changeObservation(from: oldValue, to: action, keyPath: #keyPath(XibAction.text)) { [weak self] _, _, _ in
                self?.updateText()
            }
            changeObservation(from: oldValue, to: action, keyPath: #keyPath(XibAction.image)) { [weak self] _, _, _ in
                self?.updateImage()
            }
            changeObservation(from: oldValue, to: action, keyPath: #keyPath(XibAction.color)) { [weak self] _, _, _ in
                self?.updateColor()
            }
        }
    }
